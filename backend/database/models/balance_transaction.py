...
class BalanceTransaction(Base):
    ...
    amount_usd: Mapped[Decimal] = mapped_column(
        Numeric(15, 4), nullable=False, comment='变动金额(美元)'
    )
    balance_before_usd: Mapped[Decimal] = mapped_column(
        Numeric(15, 4), nullable=False, comment='变动前余额(美元)'
    )
    balance_after_usd: Mapped[Decimal] = mapped_column(
        Numeric(15, 4), nullable=False, comment='变动后余额(美元)'
    )
    ...
    payment_id: Mapped[uuid.UUID | None] = mapped_column(
        UUID(as_uuid=True), nullable=True, comment='关联支付ID'
    )
